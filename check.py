...
class Checker:
    ...
    def check (self):
        ...
        list_response = self.service.users ().messages ().list (userId = "me", includeSpamTrash = "false", maxResults = "1", alt = "json").execute ()
        message_id = list_response ["messages"] [0] ["id"]
        get_response = self.service.users ().messages ().get (userId = "me", id = message_id, format = "raw").execute ()
